...
| **Lyria** (Gemini Live Music)  | Background music generation, changes with every scene to match the ambience.                     | `models/lyria-realtime-exp`   |
| **ChromaDB**                   | Vector store for semantic turn memory retrieval. Key for maintaining coherence over many story turns. | —                             |
| **LiveKit / Inworld TTS**      | Dialogue voice synthesis, key for immersion                        | `inworld/inworld-tts-1.5-max` |
